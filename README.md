...
採用済みの日程workbook生成処理は本番のタブ3・4、リーグ結果workbook生成処理はタブ3から利用する。
...
2026-08-18のVite production buildでは、main JavaScript `385.11 kB`（gzip `105.61 kB`）、遅延読込みする
日程workbook model `7.11 kB`（gzip `2.84 kB`）、リーグ結果workbook model `19.27 kB`
（gzip `5.93 kB`）、XLSX writer `74.02 kB`（gzip `20.81 kB`）となった。9件、合計`512.47 KiB`の
