...
決勝の配置と理由を画面・印刷・監査情報へ表示する。確定済み日程では2日目の結果を依存順に入力でき、同点時のPK戦を含む全結果を
サーバー側で再検証して、上位・下位を通した総合1位から最下位までを確定・保存・印刷できる。
トーナメント表は、各プールが8・16チームの対称な正規ブラケットなら、チーム枠を縦に並べて
勝者側を右、敗者側を左へ結ぶ水平版を表示する。それ以外は、勝者側を上、敗者側を下へ結ぶ標準版へ
フォールバックする。試合番号・開始時刻と決勝等を図へ表示し、得点・PK・勝者・最終順位を含む
完全な文字情報はアクセシブル説明と既存の一覧表で確認できる。17〜32チームの標準版は初戦概要と
最大16枠の順位帯ページへ分割し、同じ継続記号で進路を示す。画面では図だけを横スクロールでき、
水平版はA4縦、標準版はA4横のブラケット専用印刷を、保存済みデータからオフラインでも利用できる。
