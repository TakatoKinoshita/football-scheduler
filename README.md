...
現在のリポジトリでは、通常のWeb画面から大会名、4〜32チーム、ブロック数、分け方、
...
日程の生成、試合結果の入力、ブロック順位の確定、順位未確定時の仮決勝計画と
順位決定トーナメントまたは同順位リーグの作成、順位未確定時を含む2日目の時刻・コート・審判配置まで利用できる。
2日目は開催時刻を先に入力し、1つの作成操作で決勝計画と日程を続けて生成する。
...
順位確定、順位未確定でも作成できる2日目の仮決勝計画と仮日程、確定後のチーム名反映、
2日目の結果入力と総合最終順位を日別タブで確認できる。ローカル開発で日程生成を試す
場合も、Turnstileのsite keyと同一originのAPIが必要である。secret keyを`VITE_`環境変数へ
設定してはならない。

現行schema `0.2.0`の生成APIは通常画面から`request_kind: "day1_league"`を付けた大会設定を受け付ける。
...
成功した場合だけ返す。形式別の生成入口として`tournament_plan`、`day2_schedule`、
