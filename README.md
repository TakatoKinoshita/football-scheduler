...
ブラウザ内で再検証して保存する。schema `0.1.0`の保存JSONは閲覧・印刷専用で読み込み、編集用コピーでは
...
「日程設定・生成」で共通設定、1日目、2日目の順に設定を確認して、1つの操作で両日の日程を生成する。
Pythonのdomain層で
...
独立検証の集計へ保存する。1日目リーグ戦と2日目同順位リーグのチーム審判は、
直前セクションの同じコートで試合した2チームのいずれかだけから選ぶ。そのため、
試合→審判は同じコートに限る。審判→試合は、直前の試合から審判を供給した
試合→審判→試合の場合だけ許可し、後続の試合は別コートでもよい。審判→審判は禁止する。
設定休憩があっても連続セクションとみなし、空きセクションを挟んだ試合から審判を
供給しない。1日目の`team_referees_required_after_first` は互換性のため維持し、`true`では
第2セクション以降のチーム審判を必須、`false`では主催者審判への切替えを許可する。
試合→審判→試合の回数と直前同一コートからの審判数は、最適化目的ではなくハード制約の監査値として保存する。
1日目リーグ専用モデルの変数構成と計測値は
[1日目リーグ専用CP-SATモデル](docs/architecture/day1-league-compact-cp-sat.md)に記録する。

遠方から参加するチームなどには、1日目の「遠方チームの開始セクションに配慮する」で
複数チームと希望する最初のセクションを指定できる。日程の使用セクション数を増やさず、
ブロック分けも変更しない範囲で、希望より早い試合数と早さの合計を順に最小化する。
希望を完全に満たせない場合でも日程は生成し、対象チームと早い試合数を画面へ警告し、
チーム別の試合・審判監査値を保存JSONへ残す。組合せ上の調整が必要な場合は、利用者が
手動ブロック分けを選択する。
...
タブ3では全試合結果を入力して順位を確定すると、「リーグ結果をエクセルに出力」から対戦結果と
確定順位を1ブロック1sheetで端末内生成できる。得点を変更した時点で出力を無効にし、再度順位を
確定するまで以前の順位を出力しない。この出力も、保存済み結果とPWAがあればオフラインで利用できる。
決勝方式が同順位リーグの場合は、タブ4で全結果と総合最終順位を確定すると、
「同順位リーグ結果をエクセルに出力」から対戦結果、グループ内順位、総合順位、直接対戦・抽選記録を
1グループ1sheetで端末内生成できる。端数グループ、最下位統合、1チームの自動順位確定も保存内容の
まま出力し、得点の入力途中や順位失効後は再確定まで無効にする。順位決定トーナメントは対象外とする。
...
順位決定トーナメントの配置は、5トポロジー・160キーを開発時にCP-SATで事前計算した
...
主催者審判能力は使用コート数から自動的に決まり、画面では入力しない。schema `0.2.0`との互換性の
ため`organizer_capacity`は保存するが、生成要求と内部モデルでは常にコート数へ正規化する。
...
オフラインでも利用できる。
タブ3・4の印刷は画面用の折り畳みDOMを流用せず、保存済み大会から印刷snapshotを組み立てる。
大会名・保存日時、各日の組合せ概要、コート別日程、リーグ系のチーム別予定を配布順に配置し、
ボタン、入力欄、診断、監査値を印刷物へ含めない。仮日程は順位枠、順位確定後はチーム名を表示する。
...
採用済みの日程workbook生成処理は本番のタブ3・4、1日目リーグ結果workbook生成処理はタブ3、
同順位リーグ結果workbook生成処理はタブ4から利用する。
リーグ結果は全試合結果と保存済み確定順位が揃った場合だけ出力でき、クリック時に取得した1つの
保存snapshotを検証する。出力時に順位を再計算せず、エラー時も結果、順位、2日目、IndexedDBを変更しない。
生成済み日程または確定済みリーグ結果を端末へ保存していれば、再読込み後やオフラインでもdownloadできる。
2026-08-19のVite production buildでは、main JavaScript `389.85 kB`（gzip `106.65 kB`）、遅延読込みする
日程workbook model `7.11 kB`（gzip `2.84 kB`）、リーグ結果workbook model `19.99 kB`
（gzip `6.20 kB`）、同順位リーグ結果workbook model `19.27 kB`（gzip `5.56 kB`）、
XLSX writer `74.02 kB`（gzip `20.81 kB`）となった。10件、合計`536.62 KiB`の
precacheへExcel生成chunkも含め、初期表示で各modelとXLSX writerを評価せずにオフラインdownloadを維持する。
2日目同順位リーグ結果も同じ遅延読込み・download基盤を利用し、保存済み`same_rank_standings`を
正本として、`same_rank_plan`と`same_rank_league_results`との整合を独立検証してから出力する。
...
同じ入力の再現性をまとめて確認する。全160キーのcatalog整合性は、その前の`--check`で検証する。
