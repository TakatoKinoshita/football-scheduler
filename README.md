...
トーナメントは勝者・敗者の進行線を持つSVGブラケットでも表示し、仮順位枠、確定チーム名、
時刻・コート・試合番号、通常得点・PK・最終順位を同じ図へ反映する。既存の一覧表を併設し、
スマートフォンでは図だけを横スクロールできる。2日目全体に加え、上位・下位をA4横で分けた
ブラケット専用印刷も、保存済みデータからオフラインで利用できる。
