...
Issue #60で、決勝方式を「2・3・4個の順位決定トーナメント」または「同順位リーグ」から
...
この仕様の実装はIssue #61、#62、#63で追跡する。以下の「現在の実装」は、それらが完了するまで
稼働するschema `0.1.0`の動作を説明しており、任意人数トーナメント、予備戦、不戦通過、
上位・下位固定構造は新規生成の次期仕様ではない。
...
仮トーナメントはブロック順位枠を正本とし、順位確定後も組合せを変えずにチーム名を反映する。schema `0.1.0`では任意参加数の予備戦・
...
現行schema `0.1.0`の生成APIは通常画面から`request_kind: "day1_league"`を付けた大会設定を受け付ける。
