...
- 状態: 一部置換（静的配信と公開入口はADR-0002で置換）
- 決定日: 2026-08-05
- 対象: 初期リリース（MVP）
- 関連: issue #1、issue #2

2026年8月5日の対象AWS account確認でCloudFront Free定額プランへ加入できないことが
確定したため、静的配信と公開入口の決定は
[ADR-0002](adr-0002-cloudflare-pages-fallback.md)で置き換えた。Lambda、API契約、
ブラウザ保存、オフライン範囲、費用・性能の判断は引き続き有効である。

## 1. 決定

MVPは、当初次の構成を第一候補として選定した。
